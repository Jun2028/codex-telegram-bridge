...
job_slug="$(printf '%s' "$TITLE" | tr -cs 'A-Za-z0-9_-' '-' | cut -c1-32)"
job_window="job-${job_slug:-run}-$(date +%s)-$$"
tmux new-window -d -t "$SESSION" -n "$job_window" /bin/bash -lc "$WRAPPED"
tele_agent_log "started reported command in tmux session=$SESSION window=$job_window title=$TITLE"
