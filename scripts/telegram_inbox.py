...
def largest_telegram_photo(message: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return max(
        candidates,
        key=lambda item: (
            int(item.get("file_size") or 0) or int(item.get("width") or 0)
        ),
    )
...
def download_telegram_photo(
    ...
    destination_dir: Path,
    message_id: Any,
    max_bytes: int = DEFAULT_MAX_INBOUND_PHOTO_BYTES,
    timeout: int = 35,
) -> dict[str, Any]:
    if max_bytes <= 0:
    ...
    suffix = remote_suffix if remote_suffix in {".jpg", ".jpeg", ".png", ".webp"} else ".jpg"
    ...
    try:
        ...
        raise RuntimeError(f"Telegram photo download failed with HTTP {exc.code}") from None
    except (urllib.error.URLError, socket.timeout, TimeoutError) as exc:
        reason = getattr(exc, "reason", exc)
        raise TransientTelegramError(f"Telegram photo download failed: {reason}") from None
    finally:
        if temp_path is not None:
            temp_path.unlink(missing_ok=True)
    return {
...
def format_inbound_photo_text(received: dict[str, Any], caption: str) -> str:
    ...
    if caption:
        ...
        lines.append("No caption was supplied; acknowledge receipt and briefly identify the photo.")
    return "\n".join(lines)
...
def download_telegram_voice(
    ...
    destination_dir: Path,
    message_id: Any,
    max_bytes: int = DEFAULT_MAX_INBOUND_VOICE_BYTES,
    timeout: int = 35,
) -> dict[str, Any]:
    if max_bytes <= 0:
    ...
    try:
        ...
        raise RuntimeError(f"Telegram voice download failed with HTTP {exc.code}") from None
    except (urllib.error.URLError, socket.timeout, TimeoutError) as exc:
        reason = getattr(exc, "reason", exc)
        raise TransientTelegramError(f"Telegram voice download failed: {reason}") from None
    finally:
        if temp_path is not None:
            temp_path.unlink(missing_ok=True)
    return {
...
def transcribe_voice_ogg(
    ...
    try:
        ...
        if transcribed.returncode != 0:
            raise RuntimeError(f"transcription failed: {transcribed.stderr.strip()[:200]}")
    ...
    with outbox_path.open("r", encoding="utf-8") as handle:
        ...
        while True:
            ...
            if route_state_path is not None and phase == "final":
                clear_reply_route_chat_id(route_state_path, chat_id)
            offset = handle.tell()
...
def format_codex_reauth_instructions(state: dict[str, Any]) -> str:
    ...
    try:
        ...
        expires = datetime.fromtimestamp(expires_ts, LOCAL_TZ).strftime(
            "%H:%M:%S %Z"
        )
...
def format_codex_reset_confirmation(available: int, entries: list[str], expires_ts: int) -> str:
    ...
    expires_text = datetime.fromtimestamp(expires_ts, LOCAL_TZ).strftime(
        "%H:%M:%S %Z"
    )
    lines.append(
...
def format_forwarded_agent_message(
    ...
    *,
    ...
    suffix = (
        telegram_final_marker_suffix(cleaned)
        if is_final and add_final_marker
        else ""
    )
    ...
    with session_path.open("rb") as handle:
        ...
        while True:
            ...
            if not raw_text:
                offset = next_offset
                continue
            if is_group_route and phase != "final_answer":
                offset = next_offset
                continue
            max_chars = (
                max_group_final_chars
                if is_group_route and phase == "final_answer"
                else (max_final_chars if phase == "final_answer" else max_commentary_chars)
            )
            outgoing, truncated = format_forwarded_agent_message(
                ...
                env,
                max_chars,
                add_final_marker=not is_group_route,
            )
...
def codex_session_model_and_reasoning_effort(
    ...
    with handle:
        ...
        for raw_line in handle:
            ...
            if not isinstance(payload, dict):
                continue
            if record.get("type") != "turn_context" and payload.get("type") != "turn_context":
                continue
...
def format_system_status(
    ...
    if args is not None:
        ...
        if state_text:
            drain_state = read_json_object(Path(state_text))
            try:
                anchor_age = time.time() - float(
                    drain_state.get("anchor_ts") or 0
                )
            except (TypeError, ValueError):
...
def timed_message_due_label(task: dict[str, Any]) -> str:
    try:
        ...
        if not math.isfinite(due_ts):
            raise ValueError
        return datetime.fromtimestamp(due_ts, LOCAL_TZ).strftime(
            "%Y-%m-%d %H:%M:%S %Z"
        )
    except (TypeError, ValueError, OSError, OverflowError):
        return "unknown"
...
def message_mentions_bot(message: dict[str, Any], bot_username: str) -> bool:
    """True when a message is addressed to the bot by @username or command."""
    if not bot_username:
        return False
    ...
    for entity in message.get("entities") or []:
        ...
        try:
            ...
            continue
        fragment = text[start : start + length].lower()
        if entity.get("type") == "mention" and fragment == f"@{username}":
            return True
        if entity.get("type") == "bot_command" and f"@{username}" in fragment:
            return True
    return False
...
def group_owner_is_member(
    ...
    ttl_seconds: float = GROUP_OWNER_MEMBER_TTL_SECONDS,
) -> bool:
...
def resolve_source_chat(
    ...
    if chat_type == "private" and chat_id == allowed_chat_id:
        return chat_id, False, True
    return None, False, False


def read_reply_route_state(state_path: Path) -> tuple[str | None, bool]:
    """Latest chat that should receive agent replies, if still fresh.

    Returns (chat_id, is_group). A stale or missing route resolves to
    (None, False), letting callers fall back to the owner's private chat.
    """
    state = read_json_object(state_path)
    try:
        ...
        updated_ts = 0.0
    if not updated_ts or time.time() - updated_ts > REPLY_ROUTE_MAX_AGE_SECONDS:
        return None, False
    chat_id = state.get("chat_id")
    return (str(chat_id) if chat_id else None), bool(state.get("is_group"))


def set_reply_route_chat_id(state_path: Path, chat_id: str, *, is_group: bool) -> None:
    write_json_object(
        state_path,
        {
            "chat_id": chat_id,
...
def clear_reply_route_chat_id(state_path: Path, chat_id: str | None = None) -> None:
    state = read_json_object(state_path)
...
def handle_update(
    ...
    *,
    ...
    if not message:
        return
    source_chat_id, is_group, group_owner = resolve_source_chat(
        update,
        allowed_chat_id,
        owner_user_id,
        bot_username,
    ...
    photo = largest_telegram_photo(message)
    ...
    if is_group and not group_owner:
        text = strip_bot_mention(text, bot_username)
        command, payload = ("(agent-message)", text)
    else:
        command, payload = (
            ("(agent-message)", text)
            if has_attachment
            else normalize_command(text)
        )
    ...
    if command == "/codex_usage":
        ...
        try:
            ...
            if subcommand.lower() == "list":
                ...
                due = datetime.fromtimestamp(float(task["due_ts"]), LOCAL_TZ)
                heading = (
                ...
                reply = (
                    f"{heading}\n\n"
                    f"Due: `{due.strftime('%Y-%m-%d %H:%M:%S %Z')}`\n"
        ...
        if auth_failure:
            try:
                auth_detected = datetime.fromtimestamp(
                    int(auth_failure.get("detected_ts")), LOCAL_TZ
                ).strftime(" at %Y-%m-%d %H:%M:%S %Z")
            except (TypeError, ValueError, OSError):
                auth_detected = ""
        try:
            live_usage = inspect_codex_live_usage(args.repo_root)
        ...
        if voice is not None and not document_error:
            try:
                received = download_telegram_voice(
                    token,
                    voice,
                    Path(
                        getattr(
                ...
                transcript = transcribe_voice_ogg(
                    Path(received["path"]),
                    Path(getattr(args, "whisper_bin", str(DEFAULT_WHISPER_BIN))),
                    Path(getattr(args, "whisper_model", str(DEFAULT_WHISPER_MODEL))),
                    Path(getattr(args, "opusdec_bin", str(DEFAULT_OPUSDEC_BIN))),
                    Path(getattr(args, "opusdec_lib_dir", str(DEFAULT_OPUSDEC_LIB_DIR))),
                )
                record["voice"] = {**received, "transcript": transcript}
                agent_text = format_voice_text(transcript, text)
            except Exception as exc:
                reply = f"Voice note not relayed: {short_error(exc, env)}"
                record["action"] = "voice_failed"
                record["error"] = short_error(exc, env, args.max_log_chars)
                voice_error = True
        if photo is not None and not document_error and not voice_error:
            try:
                received = download_telegram_photo(
                    token,
                    photo,
                    Path(
                        getattr(
                ...
                agent_text = format_inbound_photo_text(received, text)
            except Exception as exc:
                reply = f"Could not receive photo: {short_error(exc, env)}"
        ...
        if document_error or voice_error or photo_error:
            ...
            if reply.startswith("relayed to "):
                ...
                if args.bridge_ack:
                    try:
                        send_reply(token, source_chat_id, "BRIDGE DEBUG: delivered to Codex.")
    ...
    if reply_chunks:
        for reply_chunk in reply_chunks:
            send_reply(token, source_chat_id, reply_chunk)
    elif reply:
        send_reply(token, source_chat_id, reply)
    return record
...
def dispatch_telegram_update(
    ...
    *,
    ...
    should_queue = False
    source_chat_id, _is_group, _group_owner = resolve_source_chat(
        update,
        allowed_chat_id,
        owner_user_id,
        bot_username,
    ...
    *,
    ...
    try:
        ...
        try:
            ...
            wait_for_tmux_text(args.target_pane, "Goal paused", timeout=20)
        delivery_checkpoint = codex_session_checkpoint(args.target_pane)
        ...
        if delivered:
            _anchor_agent_message_drain(args)
    finally:
        try:
...
def _anchor_agent_message_drain(
    args: argparse.Namespace,
) -> None:
    """Point the outbound drain at the root chat rollout, not a subagent file."""
    state_text = getattr(args, "agent_message_state_path", None)
    if not state_text:
        return
...
def main() -> int:
    ...
    parser.add_argument(
        "--opusdec-bin",
        default=str(DEFAULT_OPUSDEC_BIN),
        help="path to opusdec for decoding Telegram voice notes",
    )
    parser.add_argument(
        "--opusdec-lib-dir",
        default=str(DEFAULT_OPUSDEC_LIB_DIR),
    ...
    parser.add_argument(
        "--reply-route-state",
        default=str(DEFAULT_TELEGRAM_LOG_DIR / "telegram_reply_route.state.json"),
    ...
    bot_username = str(
        env_value(env, "TELEAGENT_BOT_USERNAME", "TELEGRAM_BOT_USERNAME") or ""
    ).strip().lstrip("@")
    group_member_cache: dict[str, tuple[float, bool]] = {}
    route_state_path = state_path(repo_root, args.reply_route_state)
    ...
    inbound_voice_dir = state_path(repo_root, args.inbound_voice_dir)
    ...
    inbound_photo_dir = state_path(repo_root, args.inbound_photo_dir)
    ...
    def drain_current_codex_messages() -> None:
        ...
        if not meta:
            return
        meta = agent_registry.refresh_codex_session_link(meta, target_pane=args.target_pane)
        drain_state = read_json_object(agent_message_state_path)
        try:
        ...
        if route_chat_id is None:
            route_chat_id, route_is_group = chat_id, False
