...
- Do not send command strings, qstat/df dumps, log tails, stack traces, or
...
- Reported job completion messages must still say what happened. Summaries
...
- Whenever reporting scheduled compute to Telegram, include the requested
  resources in human-readable form. Do not send raw scheduler output.
...
4. If dry-run says `"telegram": true`, send a real non-sensitive test:
   ...
   ```bash
   python3 scripts/notify.py --title "notify live test" --message "Tele-agent notification test"
   ```
...
2. Send `/newbot`.
3. Choose any display name, for example `Tele-agent Notify`.
4. Choose a unique username ending in `bot`, for example
   `tele-agent_notify_bot`.
...
Telegram, and send `Tele-agent Telegram notification test`.
...
```bash
python3 scripts/tmux_agent_report.py --title "Tele-agent status" --session tele-agent
```
...
- `scripts/tmux_agent_report.py` captures available scheduler status, scratch
