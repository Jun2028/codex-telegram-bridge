...
Each instance gets its own tmux session, scratch/log/state directories,
secret file, and Codex configuration, so several bots/Codex agents can run on
...
- model config: `config/relay.env` first, then
  `config/relay-<instance>.env` (gitignored) when it exists

Start an instance with:

```bash
TELEAGENT_INSTANCE=beta scripts/start_codex_agent.sh
TELEAGENT_INSTANCE=beta scripts/start_telegram_inbox.sh
```
...
Personality is per bot. The launcher resolves
`config/personality-<instance>.md` for non-main instances, falls back to
`config/personality.md` for the main instance, and finally to the neutral
tracked `config/personality.default.md`, then exports the resolved path as
`TELEAGENT_PERSONALITY_FILE`. Create a per-instance file to give each bot its
own voice; personality never affects relay behavior or safety rules. The
bootstrap creates the neutral default for the instance being set up.
...
The listener attaches a reply keyboard containing only `/status` to every
outgoing message, so the compact status snapshot stays available above the
input field without exposing conversational shortcuts as default buttons.

`/status` returns a compact snapshot: host and local time, desired and actual
...
agent, usage, and auth diagnostics remain under `/agent_status`; raw disk/tmux
...
Telegram photos are likewise downloaded through `getFile` into the private
`$TELEAGENT_SCRATCH/inbound_photos/` directory (default cap 10 MiB), validated
as JPEG/PNG/WebP, and relayed with their local path, dimensions, SHA-256, and
caption. Photo captions are treated like document captions, and a photo without
a caption is still relayed so Codex can inspect the image itself.
...
includes its status, due time in the host's local timezone, and a bounded message preview rendered as
...
`/timed remove` removes all active entries for the current chat. Removing stored
...
- the session cwd is the bridge repo root;
